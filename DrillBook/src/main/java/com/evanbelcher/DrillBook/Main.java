...
@SuppressWarnings("SynchronizeOnNonFinalField") public class Main {
	...
	private static final String[] versions = { "1.0.0", "1.1.0", "1.2.0", "1.2.1", "1.3.0", "1.4.0", "1.4.1", "1.5.0", "1.5.1", "1.5.2" };
	...
	/**
	 * Deletes all old DrillBook exe's
	 */
			...
			for (File f : files)
				for (int i = 0; i < versions.length - 1; i++)
					for (String extension : extensions)
						if (f.getName().equals("DrillBook.v" + versions[i] + extension))
							f.delete();
	}
	...
	private static void init() {
		...
		documentsDirPath = documentsDir.getPath();
	...
	private static void start() {
		...
		graphicsRunner.setWindowTitle("DrillBook - " + getPagesFileName());
		...
		new Thread(() -> {
			...
			while (true) {
				try {
					...
					save("autosave.show.drill", "autosave.state");
	...
	public static void load(boolean loadState) {
		if (new File(documentsDirPath + "/DrillBook/").mkdirs()) {
			state = new State(1, documentsDirPath + "/DrillBook/", "show.drill");
			saveState();
			...
			if (loadState) {
				try {
					...
					State.print("State file not found: " + documentsDirPath + "/DrillBook/" + stateFileName);
					state = new State(1, documentsDirPath + "/DrillBook/", "show.drill");
					saveState();
				}
			}
	...
	private static void loadState() throws FileNotFoundException {
		File f = new File(documentsDirPath + "/DrillBook/" + stateFileName);
	...
	public static Thread saveState(String... filename) {
		...
		Runnable r = () -> {
			synchronized (state) {
				String easyFileName = documentsDirPath + "/DrillBook/" + (filename.length == 0 ? stateFileName : filename[0]);
				...
				if (!new File(easyFileName + ".lock").exists()) {
					...
					File f = new File(documentsDirPath + "/DrillBook/" + (filename.length == 0 ? stateFileName : filename[0]));
					BufferedWriter bw;
					try {
						bw = new BufferedWriter(new FileWriter(f));
						bw.write(str);
	...
	/**
	 * Returns the file path. This maps to the DrillBook folder in Documents by default
	 */
	...
	/**
	 * Returns the file name for the pageMap json file, should include .drill
	 */
	 ...
	 *
	 * @param newPagesFileName the file name, should include .drill
	 */
