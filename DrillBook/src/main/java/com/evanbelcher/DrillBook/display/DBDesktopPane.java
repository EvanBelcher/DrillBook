 ...
 * @author Evan Belcher
 */
public class DBDesktopPane extends JDesktopPane {

	private static final long serialVersionUID = -6004681236445735439L;

	private BufferedImage fieldImage = null;
	private Dimension imgSize;
	private static DBRectangle field = new DBRectangle(25, 3, 1892 - 25, 982 - 3);
	...
	/**
	 * Constructs DBDesktopPane
	 */
	protected DBDesktopPane() {
		super();
		setFocusable(true);
	...
	private void getFieldSize() {
		...
		field = new DBRectangle(startX, startY, endX - startX, endY - startY);
		State.print(field);
	...
	@Override public void paintComponent(Graphics g) {
		...
		if (io.isShiftDragging()) {
			...
			Point q = getLocationOnScreen();
			p.translate(-q.x, -q.y);
			g2d.draw(new DBRectangle(io.getDragStart().x, io.getDragStart().y, p.x - io.getDragStart().x, p.y - io.getDragStart().y));
			g2d.setStroke(old);
		}
	}
	...
	@SuppressWarnings("unused") protected void printCurrentPage(boolean makeFolder) {
		...
		String folder = "";
		if (makeFolder)
			folder = DBMenuBar.cleanseFileName(Main.getState().getCurrentFileName().substring(0, Main.getState().getCurrentFileName().length() - 6)) + "/";

		String fileName = DBMenuBar.cleanseFileName(Main.getState().getCurrentFileName().substring(0, Main.getState().getCurrentFileName().length() - 6) + ": " + Main.getCurrentPage().toDisplayString().replaceAll("\\|", "-"));
	...
	protected void printAllPagesToPdf() throws IOException {
		...
		File f = new File(Main.getFilePath());
		f.mkdirs();
		String fileName = DBMenuBar.cleanseFileName(Main.getState().getCurrentFileName().substring(0, Main.getState().getCurrentFileName().length() - 6));
	 ...
	 */
	public static DBRectangle getField() {
		return field;
	}
