...
public class DS2MenuBar extends JMenuBar implements ActionListener {
	 ...
	 * 
	 * @param graphicsRunner
	 *            the JFrame that created this
	 * @param desktop
	 *            the DS2DesktopPane in the JFrame
	 * @since 1.0
	 */
		...
		switch (arg0.getActionCommand()) {
			...
			case "printshow":
				for (int pageNum : Main.getPages().keySet()) {
					Main.setCurrentPage(pageNum);
					desktop.printCurrentPage(true);
				}
				break;
			case "printdotsheets":
				new DotSheetMaker().printAll();
				break;
			case "togglegrid":
	 ...
	 *             if there is an error when waiting for the saves to finish
	 * @since 1.0
	 */
	 ...
	 * 
	 * @param fileName
	 *            the file name to be cleansed
	 * @param count
	 *            the count of times this has been called in the same operation.
